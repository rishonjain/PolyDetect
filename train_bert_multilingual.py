...
MODEL_DIR = "models/bert-base-multilingual-cased"   # ensure this folder exists
BATCH_SIZE = 16
os.makedirs("precomputed_features/bert_multilingual", exist_ok=True)
...
# Generator model for perplexity (local distilgpt2)
...
def get_perplexity(texts):
    ...
    for i in tqdm(range(0, len(texts), BATCH_SIZE), desc="Perplexity"):
        ...
        # out.loss is single scalar per batch — approximate per-sample by repeating
...
# ---------------- Compute / Load ----------------
train_p = cached("precomputed_features/bert_multilingual/train_perp.npy", get_perplexity, X_train.tolist())
test_p  = cached("precomputed_features/bert_multilingual/test_perp.npy", get_perplexity, X_test.tolist())
train_d = cached("precomputed_features/bert_multilingual/train_div.npy", get_div, X_train.tolist())
test_d  = cached("precomputed_features/bert_multilingual/test_div.npy", get_div, X_test.tolist())
train_e = cached("precomputed_features/bert_multilingual/train_emb.npy", get_embeds, X_train.tolist())
test_e  = cached("precomputed_features/bert_multilingual/test_emb.npy", get_embeds, X_test.tolist())
...
for n, m in models.items():
    ...
    joblib.dump(m, f"models/polydetect_bert_{n.lower()}.joblib")

df_res = pd.DataFrame(results)
df_res.to_csv("metrics_bert_multilingual.csv", index=False)

# append to combined metrics CSV (create if missing)
...
print("\n--- Results ---")
print(df_res)
print("✅ Training complete for bert-base-multilingual-cased.")
